...
- Redshiftクラスタを持っていない、アドボック、不定期ならAthenaの方がよい（Redshiftクラスタ作成はオーバーヘッドが大きくなる）
- 頻繁・定期的なクエリならSpectrumの方がパフォーマンスが安定
...
| **形式** | **圧縮率** | **処理速度** | **分割可能性 (Splittable)** | **主な用途・推奨** |
| --- | --- | --- | --- | --- |
| **Snappy** | 中 | 非常に高速 | **Yes** (Parquet/ORC内) | **DWHの標準。** パフォーマンス最優先の場合。 |
| **ZSTD** | **高** | 高速 | **Yes** (Parquet/ORC内) | **現在の推奨。** 圧縮率と速度のバランスが最高。 |
| **GZIP** | 高 | 中 | **No** (※) | ログファイル等の一般的な形式。単一ファイルだと並列化不可。 |
| **BZIP2** | 非常に高 | 低速 | **Yes** | 圧縮率重視。ただしCPU負荷が高くクエリは遅くなりがち。 |
| **LZO** | 低 | 高速 | **Yes** (インデックス付) | 過去の遺産。新規で選ぶ理由は少ない。 |
| **LZ4** | 低 | 非常に高速 | **Yes** | 極限のデコード速度が必要な場合。 |

- 分割できる形式は並列処理ができるサービス（Athenaなど）と相性がいい
- ストレージに保存する時は圧縮してコスト効率を上げる